...
const TEAM_SIZE = 5;
const ACCENTS: [string, string] = [getPlayerColor(0).hex, getPlayerColor(1).hex];
...
export default function VersusPage() {
  ...
  if (phase === 'setup') {
    return (
      <GameConfigShell
        title="Versus"
        subtitle={`Enchères carte par carte, gestion de budget et constitution de ton équipe de ${TEAM_SIZE} cartes !`}
        onBack={() => router.push('/')}
      >
        {lists.length === 0 ? (
              ...
              {[0, 1].map((i) => (
                <div key={i} className="min-w-[180px]">
                  ...
                  <PlayerNameField
                    index={i}
                    value={names[i]}
                    onChange={(v) => setNames(i === 0 ? [v, names[1]] : [names[0], v])}
                  />
                </div>
              ))}
            </div>
