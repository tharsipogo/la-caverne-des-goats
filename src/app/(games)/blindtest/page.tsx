...
import { fetchListItemMeta, shuffle } from '@/lib/utils';
...
export default function BlindTestPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const withEnough = ((data as GameList[]) || []).filter((l) => (counts.get(l.id) || 0) > 0);
      setLists(withEnough);
  ...
  return (
    <GameConfigShell
      title="Blind Test"
      subtitle="Un extrait audio minuté est joué puis révélé automatiquement. Le premier qui trouve marque un point ; premier au score gagnant remporte la partie."
      onBack={() => router.push('/')}
    >
      {lists.length === 0 ? (
