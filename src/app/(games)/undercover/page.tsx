    ...
    return (
      <div className="w-full h-full flex flex-col overflow-hidden card-enter">
        ...
        <div className="flex items-baseline gap-3 mb-4 shrink-0">
          <h1 className="font-serif text-3xl font-semibold">Undercover 🕵️</h1>
          <p className="text-sm text-muted">
            Deux mots proches sont tirés d'une base : la majorité (civils) a le même mot, la minorité
            (undercover) a l'autre. Mister White n'a rien.
          </p>
        </div>
        ...
        {lists.length === 0 ? (
          ...
          <div className="flex-1 min-h-0 flex flex-col gap-4">
            {/* Ligne de 4 cartes de config */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 shrink-0">
              <ConfigCard label="Base de mots">
              ...
              <ConfigCard label="Joueurs">
                <div className="flex items-center justify-center gap-3">
                  <CounterButton color="#3b82f6" onClick={() => setCount(playerCount - 1)}>−</CounterButton>
                  <span className="font-serif text-2xl w-8 text-center">{playerCount}</span>
                  <CounterButton color="#3b82f6" onClick={() => setCount(playerCount + 1)}>+</CounterButton>
                </div>
              </ConfigCard>

              <ConfigCard label="Undercovers">
                <div className="flex items-center justify-center gap-3">
                  <CounterButton color="#ef4444" onClick={() => setUndercoverCount(Math.max(1, undercoverCount - 1))}>−</CounterButton>
                  <span className="font-serif text-2xl w-8 text-center">{undercoverCount}</span>
                  <CounterButton color="#ef4444" onClick={() => setUndercoverCount(Math.min(maxUndercover, undercoverCount + 1))}>+</CounterButton>
                </div>
              </ConfigCard>

              <ConfigCard label="Mister White + Civils">
                <div className="flex items-center justify-between gap-2">
                  <label className="flex items-center gap-1.5 text-xs text-text cursor-pointer">
                    <input
                      type="checkbox"
                      checked={mrWhiteEnabled}
                      disabled={playerCount < 5}
                      onChange={(e) => setMrWhiteEnabled(e.target.checked)}
                    />
                    Mister White
                  </label>
                  <span className="font-serif text-2xl" style={{ color: '#f5a623' }}>{civilCount}</span>
                </div>
              </ConfigCard>
            </div>
            ...
            <div className="flex-1 min-h-0 flex flex-col">
              <label className="text-[12.5px] text-muted block mb-1.5 shrink-0">Noms des joueurs</label>
              <div className="flex-1 min-h-0 overflow-y-auto grid grid-cols-2 md:grid-cols-4 gap-2.5 content-start pr-1">
            ...
            <button className="btn w-full py-3.5 shrink-0" onClick={startGame}>
...
function ConfigCard({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div
      className="rounded-2xl p-3.5 border border-white/[0.08] flex flex-col gap-2"
      style={{ background: 'rgba(255,255,255,0.05)', boxShadow: '0 4px 0 rgba(0,0,0,0.3)' }}
    >
...
function CounterButton({ color, onClick, children }: { color: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="w-8 h-8 rounded-lg font-serif font-semibold text-sm border-none cursor-pointer transition-transform active:translate-y-0.5 active:shadow-none"
      style={{ backgroundColor: color, color: '#fff', boxShadow: `0 3px 0 ${color}99` }}
    >
      {children}
    </button>
  );
