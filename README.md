...
- **Versus** : choisis une base d'au moins 10 items, un budget de départ identique pour les deux joueurs, et leurs noms. Optionnellement, choisis une **base de coachs** (idéalement avec des images façon "carte", nom en bannière) : pour chaque joueur, tire un coach au hasard dans cette base (les deux joueurs ne peuvent jamais avoir le même coach), ou choisis-en un précisément dans une liste déroulante — affiché sous forme de carte pendant la partie et en grand dans le récap final. Optionnellement aussi, une **base de terrains** : un terrain est tiré au hasard une seule fois au début de la partie, reste affiché tout du long, et devient le fond d'écran de la partie et du récap (fond normal de l'appli si aucun terrain choisi). À chaque manche, une carte est tirée au hasard ; le joueur désigné pour commencer (l'ordre alterne à chaque manche) est obligé d'ouvrir les enchères, puis l'autre joueur peut surenchérir ou laisser la carte. La manche se termine dès qu'un joueur laisse — l'autre remporte la carte au prix de sa dernière mise. Si un joueur a 0 restant quand c'est à lui de commencer, sa mise obligatoire est automatiquement 0. La partie continue jusqu'à ce que chacun ait une équipe de 5 cartes, puis un récap stylé affiche les deux équipes (et leurs coachs) côte à côte avec un "VS" au centre.
